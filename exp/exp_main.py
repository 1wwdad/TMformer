...
from model import TMformer,TMformer1,iTransformer,Transformer,Reformer,Informer,Flowformer,Flashformer
from utils.tools import EarlyStopping, adjust_learning_rate, visual
from utils.metrics import metric,metric_components
...
class Exp_Main(Exp_Basic):
                ...
                if self.args.use_amp:
                    with torch.cuda.amp.autocast():
                        if self.args.output_attention:
                            outputs, trend_enc, seasonal_enc, trend_out, seasonal_out, attn_e, attn_d, attn = self.model(
    ...
    def test(self, setting, test=0):
        ...
        # 简化 folder_path，避免路径过长或包含中文字符
        folder_path = './test_results/' + setting + '/'
        if not os.path.exists(folder_path):
            os.makedirs(folder_path)
    
        # 初始化列表，用于积累趋势和季节性的预测和真实值
        trend_preds = []
        seasonal_preds = []
        trend_trues = []
        seasonal_trues = []
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(test_loader):
                ...
                if self.args.use_amp:
                    with torch.cuda.amp.autocast():
                        if self.args.output_attention or self.args.output_stl:
                            outputs, trend_enc, seasonal_enc, trend_out, seasonal_out, attn_e, attn_d , attn = self.model(
                                batch_x, batch_x_mark, dec_inp, batch_y_mark)
                        else:
                            outputs = self.model(batch_x, batch_x_mark, dec_inp, batch_y_mark)
